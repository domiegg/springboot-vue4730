<template>
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","fontSize":"14px","color":"#666","background":"#e9ecf3"}'>
		...
		>
			<template >
				<el-form-item :style='{"border":"0px solid #eee","padding":"0","margin":"0 0 36px 0","color":"inherit","borderRadius":"0px","width":"100%","fontSize":"inherit"}' class="input" v-if="type!='info'"  label="测试账号" prop="ceshizhanghao">
					<el-input v-model="ruleForm.ceshizhanghao" placeholder="测试账号" clearable  :readonly="ro.ceshizhanghao"></el-input>
				</el-form-item>
				<el-form-item :style='{"border":"0px solid #eee","padding":"0","margin":"0 0 36px 0","color":"inherit","borderRadius":"0px","width":"100%","fontSize":"inherit"}' v-else class="input" label="测试账号" prop="ceshizhanghao">
	...
	data() {
		...
		return {
			...
			rules: {
				...
				ceshixingming: [
					{ required: true, message: '测试姓名不能为空', trigger: 'blur' },
				],
				xingbie: [
				],
				nianling: [
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `ceshirenyuan/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.ceshirenyuanCrossAddOrUpdateFlag = false;
											this.parent.search();
